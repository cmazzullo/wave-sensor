...
#copy of function, will abstract if it works                
def dates2jd(dates):
        """
    ...
    def write_file(self, x, y, fname='-'):
        if isinstance(y, dict):
            ...
            if fname == '-':
                ...
                epoch_start = datetime.datetime(year=1970,month=1,day=1,tzinfo=pytz.UTC)
                milliseconds = [(p - epoch_start).total_seconds() * 1000 for p in x]
                depth_calc = [p for p in y]
                
                ds = Dataset(fname,'w',format="NETCDF4_CLASSIC")
                time_dimen = ds.createDimension("time",len(x))
                time = ds.createVariable("time","f8",("time",))
                time.setncattr("units","milliseconds since " + epoch_start.strftime("%Y-%m-%d %H:%M:%S"))
                time[:] = milliseconds
                depth = ds.createVariable("depth","f8",("time",))
                depth[:] = depth_calc
                ds.close()
#                 fpo = open(fname, "w")
#                 for d, v in zip(x, y):
#                     milliseconds.append(x - epoch_start).total_seconds() * 1000)
#                     depth.append(v)
...
class tappy(Util):
    ...
    def open(self, filename, def_filename = None):
        ...
        self.dates = np.array([x for x in netCDF4.num2date(nc.variables['time'][:],nc.variables['time'].units)])
        self.elevation = np.array([x for x in nc.variables['depth'][:]])
...
#                     self.inferred_phase['N2'] = phase['N2'] = phase['S2'] - 1.536*(phase['S2'] - phase['M2'])
    ...
    def filters(self, nstype, dates, elevation, pad_type=None):
        ...
        ######################################Test out boxcar filters, elminate 1 hour data manipulation 
        interval = dates[1:] - dates[:-1]
        interval.sort()
        
        #temporarily testing 1 hour data given the time step is 6min data
        dates_filled = dates[::10]
        nelevation = elevation[::10]
#         if np.any(interval < delta_dt):
#             print "Why only an hour?"
...
#                 sl = np.logical_and(dates > datetime.datetime(d.year, d.month, d.day, d.hour, tzinfo = pytz.UTC) - delta_dt/2, dates <= d + delta_dt/2)
...
#         relevation = np.empty_like(nelevation)
    ...
    def print_con(self, out_file_name):
        #graph constituents
        ndict = {}
        for k in self.key_list:
            ndict[k] = self.speed_dict[k]['speed']
        #Here we could write the data to a netCDF file if need be.
        #      nc2 = Dataset(os.path.join("TidalAnalysis.nc"),'w',format="NETCDF4_CLASSIC")
        
        #for writing a an excel file with all of the values
        names = []
        speed = []
        height = []
        phase = []
        ...
        for i in klist:
#             var = nc2.createVariable(i,"i4")
#             var.setncattr("Speed", self.speed_dict[i]['speed']*rad2deg)
#             var.setncattr("Amplitude", self.r[i])
#             var.setncattr("Phase", self.phase[i])
#             var.setncattr("Inferred", "False")
            ...
            #for excel file
            names.append(i)
            speed.append(self.speed_dict[i]['speed']*rad2deg)
            height.append(self.r[i])
            phase.append(self.phase[i])
...
#             var.setncattr("Inferred", "True")
            ...
            #for excel file
            names.append(i)
            speed.append(self.tidal_dict[i]['speed']*rad2deg)
            height.append(self.inferred_r[i])
            phase.append(self.inferred_phase[i])
            inferred.append(True)
            
            
        df = pd.DataFrame({"1. Name": pd.Series(names), "2. Speed": pd.Series(speed),
    ...
    if x.filter:
        ...
        for item in x.filter.split(','):
            if item in ['mstha', 'wavelet', 'cd', 'boxcar', 'usgs', 'doodson', 'lecolazet1', 'kalman', 'transform']:# 'lecolazet', 'godin', 'sfa']:
                ...
                filter_filename.append('outts_filtered_%s.nc' % (item,))
                x.write_file(filtered_dates, result, fname='outts_filtered_%s.nc' % (item,))
        (x.speed_dict, x.key_list) = x.which_constituents(len(x.dates),
                                                          package,
                                                          rayleigh_comp = ray)
...
def filter_subtraction(file_name, filter_name, out_filename):
    ...
    depth2 = np.interp(time1,time2,depth2);
    
    final_depth = np.subtract(depth1,depth2);
