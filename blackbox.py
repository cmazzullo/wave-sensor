...
class rbr(pressure):
        ...
        def read(self):
                ...
                '''
                f = open(self.in_filename,'rU')
                self.read_header(f) # changes the current file position
                self.data_start = self.read_datetime(f)

                data = np.genfromtxt(f,dtype=self.numpy_dtype,
                                     delimiter='    ',usecols=[1,2])
                f.close()

                long_seconds = np.int64(data["seconds"])
                self.utc_millisecond_data = (long_seconds 
					     + np.int64(self.offset_seconds)) * 1000
                print self.utc_millisecond_data[0],self.utc_millisecond_data[1]
                self.pressure_data = data["pres15"]


        def read_header(self,f):
                ''' Read the header from the RBR ASCII file. Note that
                unlike the Level Troll, the RBR doesn't put timezone 
                information in its ouput file.
                '''
                # This just moves the file position to where we need it
                for i, line in enumerate(f):
                        if line.lstrip().startswith(self.record_start_marker):
                                break
