...
$(function() {
	...
	$.getScript( "./js/query_thredds.js" )
	  .done(function( script, textStatus ) {
		...
		setTimeout( function(){
			...
			console.log(bounds._northEast['lng'] + '' + bounds._southWest['lng']);
	...
	var apply_paths = function(map)
	{
		...
		for(var i = 0; i < pairs.length)
		{
			if(pairs[i] != null)
			{
				if(last_sea != pairs[i].sea_id || last_air != pairs[i].air_id)
				{
					last_sea = pairs[i].sea_id;
					last_air = pairs[i].air_id;
