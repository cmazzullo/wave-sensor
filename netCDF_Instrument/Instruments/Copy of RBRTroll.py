...
class RBRSolo(Sensor):
    ...
    def test_16_stucksensor(self):
        ...
        self.pressure_test16_data = []
        five_count_list = dq()
        five_count_list.append(data[0])
        self.pressure_test16_data.append(1)
        
        for x in data[1:]:
            truth_count = 0
            flag_code = 1
            
            if len(five_count_list) > 5:
                five_count_list.pop()
            flags = np.equal(x,five_count_list)
            ...
            if truth_count >= 3 and truth_count < 5:
                flag_code = 3
            
            if truth_count == 5:
                flag_code = 4
            
            self.pressure_test16_data.append(flag_code)
            five_count_list.appendleft(x)
